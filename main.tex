...
%  STRUCTURE
%    1. Document class
%    2. Packages (grouped, load-order-sensitive ones noted)
%    3. Color palette
%    4. Theorem environments
%    5. Footnote & caption settings
%    6. Table of Contents tweaks
%    7. Math macros & shortcuts
%    8. Question / Solution macros
%    9. tcolorbox "sleek" styles
%   10. Custom lists & environments
...
%  Boxes, Spacing & Misc
%--------------------------------------------------------------
\usepackage{tcolorbox}      % single load; libraries loaded below
...
%  3. COLOR PALETTE
%==============================================================
\definecolor{palAmber}{HTML}{FFBA49}
\definecolor{palTeal} {HTML}{20A39E}
\definecolor{palCoral}{HTML}{EF5B5B}
\definecolor{palPlum} {HTML}{23001E}
\definecolor{palGray} {HTML}{A4A9AD}
...
\newcommand{\anglevector}[1]{\left\langle #1 \right\rangle}  % Angled Vector Notation
...
\newcommand{\jacobian}[2]{\begin{vmatrix} \frac{\partial ( #1 )}{\partial ( #2 )} \end{vmatrix}} % Jacobian
...
\tcbset{
  sleek/.style={
    ...
    colback=white,
    arc=7pt, outer arc=7pt, boxsep=0pt,
    left=20pt, right=18pt, top=15pt, bottom=15pt,
    drop fuzzy shadow southeast={palGray!70},
    fonttitle=\bfseries\sffamily, coltitle=white,
    attach boxed title to top left={xshift=16pt, yshift=-\tcboxedtitleheight/2},
    boxed title style={frame hidden, arc=5pt, boxsep=5pt, left=8pt, right=8pt},
  }
}

\newtcolorbox{sleekbox}[2][palTeal]{%
  sleek, borderline west={4pt}{0pt}{#1}, colbacktitle=#1, title={#2}}

\newtcolorbox{sleekplain}[1][palTeal]{%
  sleek, borderline west={4pt}{0pt}{#1}}
...
\section{Resources:}
\begin{itemize}
    \item \href{https://www.khanacademy.org/math/ap-calculus-bc}{Calculus I and II Review} \cite{collegeboard_apcalcbc, khanacademy_calcbc}
    \item \href{https://www.youtube.com/watch?v=NJiaIr5Y2VU&list=PLVOT1I6lLvjfDhACPSMXam2diM0IBR3hl}{Professor Forte} \cite{forte_calc3_2025}
    \item \href{https://tutorial.math.lamar.edu/Classes/CalcIII/CalcIII.aspx}{Paul's Online Math Notes} \cite{dawkins_calc3}
    \item \href{https://www.wolframcloud.com/env/19521757-1a29-4b32-9b72-33be09e19ce3}{Mathematica Notebook}
\end{itemize}

\section{Environment Demos:}

    \begin{sleekbox}[palTeal]{Definition}
        A \emph{smooth manifold} is a topological manifold equipped with an atlas whose
        transition maps are all infinitely differentiable. Accent uses \texttt{palTeal}.
    \end{sleekbox}
     
    \vspace{10pt}
    \begin{sleekbox}[palCoral]{Warning}
        The second-derivative test is inconclusive when the Hessian determinant vanishes.
        Accent uses \texttt{palCoral}.
    \end{sleekbox}
     
    \vspace{10pt}
    \begin{sleekbox}[palAmber]{Example}
        For $f(x,y)=x^2+y^2$, the gradient is $\nabla f = (2x,\,2y)$, which vanishes only
        at the origin. Accent uses \texttt{palAmber}.
    \end{sleekbox}
     
    \vspace{10pt}
        \begin{sleekbox}[palPlum]{Theorem}
        Every continuous function on a compact set attains its maximum and minimum.
        Accent uses \texttt{palPlum}.
    \end{sleekbox}
     
    \vspace{10pt}
    \begin{sleekplain}[palGray]
    A title-free variant for asides and margin-style notes. Accent uses \texttt{palGray}.
    \end{sleekplain}
...
\section{Unit 13:}
    ...
    \subsection{Labs/Homework/Discussions:}
        \paragraph{Discussion \#1 Introductions:}\leavevmode\par\medskip
            \input{Unit 13/Discussion #1 Introductions}
            \leavevmode\par\medskip
            
        \paragraph{Lab 0 - Introduction to Mathematica (not collected):}\leavevmode\par\medskip
            \input{Unit 13/Lab 0/Lab 0 - Introduction to Mathematica (not collected)}
            \leavevmode\par\medskip
            
        \paragraph{Homework 13.1 - 13.3:}\leavevmode\par\medskip
            \input{Unit 13/Homework 13.1 - 13.3}
            \leavevmode\par\medskip
            
        \paragraph{Discussion \#2 Vectors and Dot Products:}\leavevmode\par\medskip
            \input{Unit 13/Discussion #2 Vectors and Dot Products}
            \leavevmode\par\medskip
        \newpage    
        \paragraph{Lab 1 - (Sec. 13.1 - 13.5):}\leavevmode\par\medskip
            \input{Unit 13/Lab 1/Lab 1 - (Sec. 13.1 - 13.5)}
            \leavevmode\par\medskip
            
        \paragraph{Homework 13.4 - 13.6:}\leavevmode\par\medskip
            \input{Unit 13/Homework 13.4 - 13.6}
            \leavevmode\par\medskip
            
        \paragraph{Discussion \#3 Potato Chips and 3D Surfaces:}\leavevmode\par\medskip
            \input{Unit 13/Discussion #3 Potato Chips and 3D Surfaces}
            \leavevmode\par\medskip
...
\section{Unit 14:}
    \subsection{Lessons:}
        \begin{itemize}
            ...
            \item \href{https://www.youtube.com/watch?v=hUq-uHI7Mpw&list=PLVOT1I6lLvjfDhACPSMXam2diM0IBR3hl&index=6}{Curvature and Normal Vectors Pt.1}
